...
- Taxonomy values must be exact items from the registration phase lists. If you want taxonomy, load `phases/phase-registration.md`; do not call `/api/taxonomy`, schema, option, discovery, or taxonomy files. Do not invent snake_case summaries; map human detail to the nearest listed value and keep specifics in prose.
